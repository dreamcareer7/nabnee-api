...
module.exports.register_resp = function(status, data) {
	if(status === "Registered") {
		...
		let token = jwt.sign({user_email: data.user_email,userId:data._id},ConstantCtrl.JWT_SECRET,{ expiresIn: "365d" });
		return {success : true, message : "User logged in successfully", user_details : [data], user_token : token};
	}
...
module.exports.user_operation_responses = function(key, data) {
	if(key === "Updatedwithtoken") {
		...
		return {success : true, message : "User data updated", user_details : data,user_token : token};
	}
	if(key === "Updated") {
		return {success : true, message : "User data updated", user_details : [data]};
	}
